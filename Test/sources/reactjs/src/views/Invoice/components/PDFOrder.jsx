...
function productPDF(data, subData, t, nameBranch) {
  let dataColumn = [[
    ...
    { text: t('Mã đơn hàng'), style: 'tableHeader' },
  ...
  for (let item of subData) {
    ...
    dataColumn.push(
      [
        ...
        { text: item.customerId ? item.customerId.name : "" },
        { text: item.finalAmount ? ExtendFunction.FormatNumber(item.finalAmount) : 0, style: 'number' },
        { text: t(Constants.INVOICE_STATUS.name[item.status]) },
      ]);
  }
  ...
  let documentDefinition =
  {
    content:
      [
        ...
        { text: t("Danh sách đơn hàng"), bold: true },
        {
          table: {
            headerRows: 1,
            body: dataColumn,
  ...
  pdfMake.createPdf(documentDefinition).download(t('Danhsachdonhang.pdf'));
}
