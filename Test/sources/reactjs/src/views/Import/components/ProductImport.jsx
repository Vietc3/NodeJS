      ...
      },
      {
        ...
        render: (value, record, index) => {
          ...
          return (
            ...
            <Popover
                ...
                content={
                  <DiscountUnitPriceForm
                    onChange={(formData) => {
                      let item = ProductsForm[index];
                      item.unitPrice = formData.importPrice;

                      if(formData.discountType === Constants.DISCOUNT_TYPES.id.percent)
                        item.discount = formData.discount * formData.importPrice / 100;
                      else
                        item.discount = formData.discount;
                      if(item.discount > formData.importPrice){
                        notifyError("Chiết khấu không thể lớn hón giá nhập");
                        item.finalAmount = 0;
                      }
                      else{
                        item.finalAmount = formData.importPrice - item.discount ;
                      }
                      item.total = item.finalAmount * item.quantity;
            ...
            {record.finalAmount !== record.unitPrice && record.unitPrice !== 0 && (
                ...
                <Container style={{ paddingBottom: 20 }}>
                  ...
                  <Row>
                    ...
                    {isCanceledCard || isReturn ? this.getTextFieldDiscount(this.state.taxAmount, true) :
                      ...
                      >
                        {this.getTextFieldDiscount(this.state.taxAmount, false)}
                      </Popover>
                    }
                    </Col>
                  </Row>
