...
function init_forms(){ 
	...
	*/	
   if($("select, input:radio, input:file").size()){
...
function init_wysiwyg() {
   ...
   $('textarea.wysiwyg-editor').each(function(){
      ...
      new nicEditor({
         iconsPath : '_layout/scripts/nicEdit/nicEditorIcons.gif'
      }).panelInstance(editor_id); 
...
function init_panels() {
   ...
   $('.panel .collapse').click(function(){
      if ($(this).closest('.panel').hasClass('collapsed')){
         ...
         $(this).closest('.panel').addClass('collapsed').animate({
            height:'45px'
         }, function(){		});
      }
   }); 
