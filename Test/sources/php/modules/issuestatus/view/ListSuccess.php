...
<script type="text/javascript">
    ...
    function getList() {
        ...
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/GetAllIssueStatus?rows="); ?>'+rows,
            async: false
        }).responseText;
        $("#sample-table-sortable").html(result);
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/GetAllIssueStatusCount?rows="); ?>'+rows,
            async: false
        }).responseText;
    ...
    function backpage() {
        ...
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/GetAllIssueStatus?rows="); ?>'+rows+'&page='+page,
            async: false
        }).responseText;
        $("#sample-table-sortable").html(result);
    }
    ...
    function deletestatus_issue(id) {
        $confirm = confirm('Are you sure you want to delete?');
        if($confirm){
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/DeleteIssueStatus?id="); ?>'+id,
            async: false
        }).responseText;
        ...
        {
            alert("Issue status is in used, can't be deleted!");
        }
        }
    }
</script>
...
<div class="row">
    <div class="span12">
        <div class="panel-wrapper">
            <div class="panel">
                <div class="title">
                    <h4>List Issue Status
                        <div style="float: right; padding-top: 9px; padding-right: 10px;">
                            <a class="btn btn-success btn-title" href="<?php echo link_to("issuestatus/edit"); ?>">
                ...
                <table id="sample-table-sortable" class="table sortable">
                    ...
                    <tbody id="tbody">
                        <?php if(count($this->issue_statuses)>0) 
                            { 
                                foreach ($this->issue_statuses as $issue_status) 
                                { ?>
                                    <tr style="background-color: <?php echo $issue_status->getColor(); ?>">
                                        <td><a href="<?php echo link_to("issuestatus/edit"); ?>&id=<?php echo $issue_status->getId(); ?>"><?php echo $issue_status->getName(); ?></a></td>
                                        <td><?php echo $issue_status->getDescription(); ?></td>
                                        <td><?php echo $issue_status->getOrderId(); ?></td>
                                        <td class="center">
                                            <a title="Delete" href="javascript:deletestatus_issue(<?php echo $issue_status->getId(); ?>)"><img src="<?php echo href_to('templates/quickadmin/_layout/images/icons/delete.png') ?>"></a>
                                        </td>
                                    </tr>
                    <?php
                                }
