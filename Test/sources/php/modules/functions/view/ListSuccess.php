...
<script type="text/javascript">
    ...
    function getList() {
        ...
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/GetAllFunction?rows="); ?>'+rows,
            async: false
        }).responseText;
        $("#sample-table-sortable").html(result);
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/GetAllFunctionCount?rows="); ?>'+rows,
            async: false
        }).responseText;
    ...
    function backpage() {
        ...
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/GetAllFunction?rows="); ?>'+rows+'&page='+page,
            async: false
        }).responseText;
        $("#sample-table-sortable").html(result);
    }
    ...
    function deletefunction(id) {
        ...
        var result = $.ajax({
            type: 'POST',
            url: '<?php echo link_to("ajax/DeleteFunction?id="); ?>'+id,
            async: false
        }).responseText;
        ...
        {
            alert("Functions is in used, can't be deleted!");
        }
        }
    }
</script>
                ...
                <div class="title">
                    <h4>List Functions
                        <div style="float: right; padding-top: 9px; padding-right: 10px;">
                            <a class="btn btn-success" href="<?php echo link_to("functions/edit"); ?>">
                                <img alt="" src="<?php echo href_to('templates/quickadmin/_layout/images/icons/document-new.png') ?>"/> Create Function
                            </a>
                        </div>
                    </h4>
                    ...
                    <tbody id="tbody">
                        <?php if (count($this->functions) > 0) {
                            foreach ($this->functions as $function) : ?>
                                <tr>
                                    <td><?php echo $function->getName(); ?></a></td>
                                    <td><?php echo $function->getDescription(); ?></td>
                                    <td><?php echo $function->getCategory(); ?></td>
                                </tr>
                            <?php endforeach;
                        } else { ?>
                            <tr><td style="text-align:center" colspan="3">There is no available functions!</td></tr>
<?php } ?>
                    </tbody>
                </table>
            </div>
