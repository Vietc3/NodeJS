module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    if(foundCustomer.totalDeposit < amount) {
    ...
    let createdDepositCard = await DepositCard.create(req, _.pickBy({
      code,
      type: sails.config.constant.DEPOSIT_TYPES.WITHDRAW,
      ...
      updatedBy: createdBy,
      notes: notes || (originalVoucherCode ? (sails.config.constant.autoWithdrawDeposit + originalVoucherCode) : ""),
      branchId: branchId
    ...
    if (isActionLog) {
      ...
      let createActionLog = await sails.helpers.actionLog.create(req, {
        userId: createdBy,
        functionNumber: sails.config.constant.ACTION_LOG_TYPE.WITHDRAW_DEPOSIT,
